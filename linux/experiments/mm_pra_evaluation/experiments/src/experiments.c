...
int main(int argc, char *argv[])
{
  ...
  if (mapFile(&test_exec_file_mapping, TEST_EXEC_FILE_PATH, FILE_ACCESS_READ, MAPPING_ACCESS_READ | MAPPING_ACCESS_EXECUTE | MAPPING_SHARED) != 0)
  {
  ...
  // map eviction memory
  if (mapFile(&eviction_file_mapping, EVICTION_FILENAME, FILE_ACCESS_READ, MAPPING_ACCESS_READ | MAPPING_ACCESS_EXECUTE | MAPPING_SHARED) != 0)
  {
  ...
  if (mapFile(&eviction2_file_mapping, EVICTION2_FILENAME, FILE_ACCESS_READ, MAPPING_ACCESS_READ | MAPPING_ACCESS_EXECUTE | MAPPING_SHARED) != 0)
  {
    printf("Error (%s) at mapFile for: %s ...\n", strerror(errno), EVICTION_FILENAME);
    goto error;
  }
