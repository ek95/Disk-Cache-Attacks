...
int main(int argc, char *argv[])
{
  ...
  // map test file
  if (mapFile(&file_mapping, TEST_FILE_PATH, FILE_ACCESS_READ, MAPPING_ACCESS_READ | MAPPING_SHARED) != 0)
  {
    printf("Error (%s) at mapFile for: %s ...\n", strerror(errno), TEST_FILE_PATH);
    goto error;
  }
#ifdef _READ_NONBLOCK_
  int flags = fcntl(file_mapping.internal_.fd_, F_GETFL, 0);
  if (flags == -1)
  {
    printf("Error (%s) at fcntl ...\n", strerror(errno));
    goto error;
  }
  if (fcntl(file_mapping.internal_.fd_, F_SETFL, flags | O_NONBLOCK) == -1)
  {
    printf("Error (%s) at fcntl ...\n", strerror(errno));
    goto error;
  }
  ...
  posix_fadvise(file_mapping.internal_.fd_, TEST_FILE_TARGET_PAGE * PAGE_SIZE, PAGE_SIZE, POSIX_FADV_DONTNEED);
  ...
  while (1)
  {
    ...
    {
      if (pread(file_mapping.internal_.fd_, (uint8_t *)&tmp, 1, TEST_FILE_TARGET_PAGE * PAGE_SIZE) != 1)
      {
    ...
    else if (choice == 's')
    {
      ...
      ret_pread = preadv2(file_mapping.internal_.fd_, &io_range, 1, TEST_FILE_TARGET_PAGE * PAGE_SIZE, RWF_NOWAIT);
      if (ret_pread == -1)
      {
        if (errno == EAGAIN)
        {
      ...
      ret_pread = pread(file_mapping.internal_.fd_, (void *)&tmp, 1, TEST_FILE_TARGET_PAGE * PAGE_SIZE);
      if (ret_pread == -1)
      {
        if (errno == EAGAIN)
        {
